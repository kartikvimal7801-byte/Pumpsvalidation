...
export default function NPDCategoryProjects() {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await projectService.getProjectsByModule('npd');
  ...
  const handleOpenProject = (project: Project) => {
    navigate(`/npd/project/${project.id}`);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
          ...
          className="flex items-center gap-4 mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl"
        >
          <div className="w-16 h-16 flex-shrink-0 bg-white rounded-xl border border-blue-100 flex items-center justify-center overflow-hidden shadow-sm">
            <img
              src={category.image}
              alt={category.name}
              className="max-w-full max-h-full object-contain"
              ...
              <Card className="p-12 text-center">
                <div className="max-w-sm mx-auto">
                  ...
                  <p className="text-gray-500 text-sm mb-6">
                    {allProjects.length === 0
                      ? `Create the first NPD project for ${category.name}.`
