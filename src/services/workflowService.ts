...
// NEW SIMPLIFIED CRITERIA: Only 3 validation stages for all nodes
...
function buildDefaultCriteria(nodeId: string): ValidationCriterion[] {
  return STANDARD_CRITERIA.map((label, i) => ({
    id: `${nodeId}-crit-${i}`,
    label,
    required: true,
  ...
  getWorkflowState(projectId: string): WorkflowState {
    ...
    if (saved?.stages) {
      ...
      for (const id of ALL_NODE_IDS) {
        ...
        // Hydrate reviewState for older saved data
        if (!stages[id].reviewState) stages[id].reviewState = 'idle';
      }
      return { ...saved, stages };
    }
