...
const categories = getPumpCategoriesForModule('npd');
...
export default function NPDProjects() {
  ...
  useEffect(() => {
    projectService.getProjectsByModule('npd').then((res) => {
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16 gap-3">
            ...
            <span className="text-xs font-bold text-white px-2 py-0.5 rounded bg-blue-600">
              NPD
            </span>
            <span className="text-sm text-gray-500">New Product Development</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm">
            Click a pump category to view and manage its NPD projects
          </p>
        </div>
          ...
          {categories.map((category, index) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => navigate(`/npd/${category.id}`)}
