...
export default function StandardizationProjects() {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await projectService.getProjectsByModule('standardization');
  ...
  const handleCreateProject = async (data: ProjectFormData) => {
    ...
    try {
      ...
      const response = await projectService.createProject(
        { name: data.name, owner: data.owner, startDate: data.startDate, description: data.description, moduleType: 'standardization', pumpCategory: selectedCategory ?? undefined },
        user.id
      );
  ...
  const handleOpenProject = (project: Project) => navigate(`/workspace/standardization/${project.id}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-bold text-gray-900">Standardization Projects</h1>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Standardization Projects</h2>
          <p className="text-gray-600">Establish and maintain standardized processes across all product development activities.</p>
        </div>
            ...
            {filteredProjects.length > 0 ? (
              ...
              <Card className="p-12 text-center">
                <div className="max-w-sm mx-auto">
                  ...
                  <p className="text-gray-500 mb-6">{projects.length === 0 ? 'Get started by creating your first Standardization project.' : 'Try adjusting your search or filter criteria.'}</p>
      ...
      <ProjectForm isOpen={showCreateForm} onClose={() => { setShowCreateForm(false); setSelectedCategory(null); setSelectedCategoryObj(null); }} onSubmit={handleCreateProject} moduleType="standardization" isLoading={formLoading} pumpCategory={selectedCategoryObj} />
      <ProjectForm isOpen={!!editingProject} onClose={() => setEditingProject(null)} onSubmit={handleEditProject} project={editingProject} moduleType="standardization" isLoading={formLoading} />
    </div>
  );
}
